...
def upgrade() -> None:
    ...
    NOTE: ENUM types are created automatically by op.create_table
    from the sa.Enum columns below (each enum type is used by exactly
    one table). Do NOT create them explicitly beforehand — that emits
    a duplicate CREATE TYPE and fails.
    """
    # --- TENANTS ---
    op.create_table(
        "tenants",
...
def downgrade() -> None:
    ...
    # Drop ENUM types explicitly. PostgreSQL does NOT auto-drop enum
    # types when their referencing tables are dropped (SQLAlchemy emits
    # CREATE TYPE separately from CREATE TABLE), so these explicit drops
    # are load-bearing — do not remove them, or an upgrade→downgrade→
    # upgrade cycle fails with a duplicate CREATE TYPE error.
