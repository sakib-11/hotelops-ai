"""Tests for Task 16.7 — the deterministic service_gap_candidate rule.

Converts an OPEN canonical ``WaitingInterval`` (Task 15.5.3) in a
configured service area into a ``service_gap_candidate`` event when the
configured service-gap grace duration is crossed. The rule does NOT
inspect raw frames, YOLO detections, tracker objects, bounding boxes,
pixels, or video files — it consumes the confirmed canonical waiting fact
+ its canonical spatial context only.

Covered (Task 16.7 Part 40):

- unit: valid service gap, below / exact / above threshold, service
  resumed, invalid service context, missing service context, invalid
  duration, missing/invalid configuration, missing/invalid event_time;
- boundary: threshold - 1 (NO_MATCH), threshold exactly (MATCH),
  threshold + 1 (MATCH) — the documented boundary policy;
- temporal: continuous gap (single qualification), short interruption,
  service resumption, re-entry, late/out-of-order per Task 15 policy;
- idempotency: repeated evaluation, duplicate fact, replay;
...
- invariants (Part 41): 15 guarantees;
- golden tests (§26-35).
...
_RULE_ID = RuleId(RuleIdentifier.SERVICE_GAP_CANDIDATE.value)
_RULE_VERSION = RuleVersion("v1")
_THRESHOLD = 300.0  # the configured service-gap grace threshold
_SERVICE_AREA = "service-reception"
_OTHER_AREA = "zone-lobby"  # a waiting-capable context that is NOT a service area
...
def _key(
    *,
    ...
    semantic_context: str | None = _SERVICE_AREA,
) -> TemporalStateKey:
...
def _input(
    ...
    *,
    ...
    return RuleEvaluationInput(
        facts=(interval,),
        configuration={
            SERVICE_GAP_CONFIG_KEY: threshold,
            SERVICE_AREA_IDS_CONFIG_KEY: (
                service_areas if service_areas is not None else [_SERVICE_AREA]
            ),
        },
...
def _engine():
    """The sanctioned operational engine (service_gap_candidate:v1 registered)."""
...
class TestUnit:
    def test_valid_service_gap_match(self) -> None:
        result = _evaluate(_interval(duration=300.0))
        assert result.status is RuleEvaluationStatus.MATCH
        assert result.event is not None
        assert result.event.event_type == RuleEventType.SERVICE_GAP_CANDIDATE.value
        assert result.event.event_time == _event(300)
        payload = result.event.payload
        assert payload.gap_duration == 300
        assert payload.threshold_seconds == 300
        assert payload.gap_start_time == _START
        ...
        assert payload.service_area_id == _SERVICE_AREA
        assert payload.configuration_version_id == _CONFIG_V1
        assert payload.rule_id == RuleIdentifier.SERVICE_GAP_CANDIDATE.value
    ...
    def test_exact_threshold_match(self) -> None:
        ...
        assert result.status is RuleEvaluationStatus.MATCH
        assert result.event is not None
        assert result.event.payload.gap_duration == 300

    def test_above_threshold_match(self) -> None:
        result = _evaluate(_interval(duration=301.0))
        assert result.status is RuleEvaluationStatus.MATCH
        assert result.event is not None
        assert result.event.payload.gap_duration == 301

    def test_service_resumed_no_match(self) -> None:
        # Part 15: a CLOSED interval (service resumed / gap ended) never
        # generates a service-gap event.
    ...
    def test_invalid_service_context_no_match(self) -> None:
        # A waiting fact OUTSIDE the configured service areas is NO_MATCH —
        # never silently classified as a service gap.
        result = _evaluate(
            _interval(duration=300.0, key=_key(semantic_context=_OTHER_AREA)),
            service_areas=[_SERVICE_AREA],
        )
        assert result.status is RuleEvaluationStatus.NO_MATCH
        assert result.event is None

    def test_missing_service_context_is_invalid(self) -> None:
        # A waiting fact with NO spatial context cannot be a service gap
        # (Part 10: missing service context).
    ...
    def test_missing_service_area_config_rejected(self) -> None:
        engine = _engine()
        inp = _input(_interval(duration=300.0)).model_copy(
            update={"configuration": {SERVICE_GAP_CONFIG_KEY: 300.0}}
        )
        with pytest.raises(MissingRuleConfigurationError, match="service_area_ids"):
            engine.evaluate(_RULE_ID, _RULE_VERSION, inp)

    def test_invalid_service_area_list_value_is_invalid(self) -> None:
        engine = _engine()
        inp = _input(_interval(duration=300.0), service_areas=[])
        ...
        inp_bad = _input(_interval(duration=300.0)).model_copy(
            update={
                "configuration": {
                    SERVICE_GAP_CONFIG_KEY: 300.0,
                    SERVICE_AREA_IDS_CONFIG_KEY: ["valid", 123],
                }
            }
        )
    ...
    def test_invalid_threshold_non_numeric_is_invalid(self) -> None:
        ...
        inp = _input(_interval(duration=300.0)).model_copy(
            update={
                "configuration": {
                    SERVICE_GAP_CONFIG_KEY: "abc",
                    SERVICE_AREA_IDS_CONFIG_KEY: [_SERVICE_AREA],
                }
            }
        )
        result = engine.evaluate(_RULE_ID, _RULE_VERSION, inp)
    ...
    def test_missing_configuration_rejected(self) -> None:
        ...
        with pytest.raises(MissingRuleConfigurationError, match=SERVICE_GAP_CONFIG_KEY):
            engine.evaluate(_RULE_ID, _RULE_VERSION, inp)
    ...
    def test_missing_event_time_rejected_at_contract(self) -> None:
        ...
        with pytest.raises(ValidationError, match="event_time"):
            RuleEvaluationInput(
                facts=(interval,),
                configuration={SERVICE_GAP_CONFIG_KEY: 300.0},
...
class TestTemporal:
    def test_continuous_gap_single_qualification(self) -> None:
        # Timeline (Part 12): below, crossing, still gap. The rule fires
        # ONCE for the crossing fact; re-evaluating it stays one event.
        interval_id = EventId(uuid.uuid5(uuid.NAMESPACE_URL, "gap-episode-A"))
    ...
    def test_short_interruption_no_duplicate(self) -> None:
        # Task 15.5.3 keeps the interval ALIVE across short interruption
        # (occlusion); re-feeding the crossing fact produces one event
        # (Part 16).
        interval = _interval(duration=300.0)
    ...
    def test_service_resumption_stops_events(self) -> None:
        # Part 15: once the interval closes (service resumed) the rule
        # returns NO_MATCH — no further events after resumption.
        reopened = _interval(duration=300.0, reason=TemporalReason.EXIT_CONFIRMED, label="closed")
        result = _evaluate(reopened)
        assert result.status is RuleEvaluationStatus.NO_MATCH
        assert result.event is None

    def test_reentry_after_resumption_independent(self) -> None:
        # Episode A qualifies and closes; Episode B later qualifies → two
        ...
        ra = _evaluate(a)
        rb = _evaluate(b)
        assert ra.status is RuleEvaluationStatus.NO_MATCH  # A is closed (resumed)
        assert rb.status is RuleEvaluationStatus.MATCH
        assert rb.event is not None
        assert rb.event.payload.gap_start_time == _event(1200)
    ...
    def test_late_event_consumed_per_task15_policy(self) -> None:
        ...
        # a fact — a fact Task 15 rejects never reaches the rule (Part 34).
        interval = _interval(duration=300.0)
        result = _evaluate(interval)
        assert result.status is RuleEvaluationStatus.MATCH
        assert result.event is not None
...
class TestIdempotency:
    ...
    def test_duplicate_fact_same_event(self) -> None:
...
class TestVersions:
    def test_v1_resolves(self) -> None:
        ...
        assert rule.evaluator_id == SERVICE_GAP_EVALUATOR_ID
    ...
    def test_historical_v1_replay(self) -> None:
        ...
        # (no latest-configuration lookup; Part 18/33).
        interval = _interval(duration=301.0)
...
class TestContracts:
    def test_envelope_serializes_round_trip(self) -> None:
        ...
        assert restored.event_type == RuleEventType.SERVICE_GAP_CANDIDATE.value
        assert restored.event_time == _event(300)
        assert restored.schema_version == "1.0"
        payload = ServiceGapCandidatePayload.model_validate(restored.payload)
    ...
    def test_evidence_request_preserves_context(self) -> None:
        ...
        assert ref.metadata["rule_id"] == RuleIdentifier.SERVICE_GAP_CANDIDATE.value
...
# =============================================================================
# 38/41. OBSERVABILITY + INVARIANT TESTS
# =============================================================================
...
class TestObservability:
    def test_match_records_structured_telemetry(self, caplog) -> None:
        ...
        msg = rec.getMessage()
        assert f"rule_id={RuleIdentifier.SERVICE_GAP_CANDIDATE.value}" in msg
        ...
        # Service-gap measurements are recorded via the payload.
        assert "gap_duration" in msg and "300" in msg
        assert "threshold_seconds" in msg and "300" in msg
        assert "service_area_id" in msg and _SERVICE_AREA in msg
...
class TestInvariants:
    ...
    def test_one_continuous_gap_no_uncontrolled_duplicates(self) -> None:
        interval = _interval(duration=300.0)
        ids = {_evaluate(interval).event.event_id for _ in range(5)}
        assert len(ids) == 1

    def test_service_resumption_closes_candidate(self) -> None:
        closed = _interval(duration=300.0, reason=TemporalReason.EXIT_CONFIRMED, label="closed")
        assert _evaluate(closed).status is RuleEvaluationStatus.NO_MATCH

    def test_new_episode_new_logical_candidate(self) -> None:
    ...
    def test_rule_version_preserved(self) -> None:
        result = _evaluate(_interval(duration=300.0))
        assert result.rule_id == RuleIdentifier.SERVICE_GAP_CANDIDATE.value
    ...
    def test_rule_does_not_access_raw_video(self) -> None:
        ...
        ev = ServiceGapCandidateEvaluator()
...
class TestGolden:
    def test_golden_below_threshold(self) -> None:
        """§26: threshold 300s, gap 299s → NO_MATCH."""
        result = _evaluate(_interval(duration=299.0))
        assert result.status is RuleEvaluationStatus.NO_MATCH

    def test_golden_exact_threshold(self) -> None:
        """§27: threshold 300s, gap exactly 300s → MATCH (mandatory)."""
        result = _evaluate(_interval(duration=300.0))
        assert result.status is RuleEvaluationStatus.MATCH

    def test_golden_above_threshold(self) -> None:
        """§28: threshold 300s, gap 301s → MATCH."""
        result = _evaluate(_interval(duration=301.0))
        assert result.status is RuleEvaluationStatus.MATCH

    def test_golden_continuous_gap(self) -> None:
        """§29: 10:00 gap starts, 10:04:59 below, 10:05:00 crossed, still
        gap at 10:06/10:07 → exactly ONE logical event for the crossing."""
        interval_id = EventId(uuid.uuid5(uuid.NAMESPACE_URL, "gap-episode-A"))
        crossing = _interval(duration=300.0, last_seen=_event(300), interval_id=interval_id)
    ...
    def test_golden_service_resumption(self) -> None:
        """§30: 10:00 gap starts, 10:05 crossed, 10:06 service resumes →
        ONE event; no further events after resumption."""
        ...
        # After resumption the fact is CLOSED → NO_MATCH (no further events).
        assert _evaluate(resumed).status is RuleEvaluationStatus.NO_MATCH
        assert _evaluate(resumed).event is None

    def test_golden_reentry(self) -> None:
        """§31: Episode A qualifies + resumes; Episode B qualifies → TWO
        independent events."""
        ...
        assert ra.status is RuleEvaluationStatus.NO_MATCH  # A closed (resumed)
        assert rb.status is RuleEvaluationStatus.MATCH
        assert rb.event is not None
        assert ra.event is None  # no event after resumption
        assert rb.event.event_id is not None

    def test_golden_replay(self) -> None:
        """§32: identical input evaluated multiple times → identical
        result, event type, event_time, payload, versions, identity."""
        interval = _interval(duration=300.0)
        engine = _engine()
        inp = _input(interval)
    ...
    def test_golden_configuration_replay(self) -> None:
        """§33: config v1 then v2; replay the historical fact explicitly
    ...
    def test_golden_late_event(self) -> None:
        """§34: a fact Task 15 accepted under its late policy is consumed
    ...
    def test_golden_out_of_order_event(self) -> None:
        """§35: the rule has no temporal-ordering role — it follows Task
        15's event-time policy by consuming canonical facts only."""
        interval = _interval(duration=300.0)
        result = _evaluate(interval)
        assert result.status is RuleEvaluationStatus.MATCH
        assert result.event.event_time == _event(300)
