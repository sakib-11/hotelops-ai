...
Applies the ``security`` marker to every test collected under this
directory so that ``pytest -m security`` selects the security tests as
...
def pytest_collection_modifyitems(config: pytest.Config, items: list[pytest.Item]) -> None:
    """Attach the security marker to every test collected in this directory."""
